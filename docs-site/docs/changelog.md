...
<div class="badge-row">
<span class="sate-badge">recorder 1.5.20</span>
...
timeline
  ...
  2026-07-23 : 1.5.15 remote stop : 1.5.16 remote takes resume : 1.5.17 resume keeps the network up : 1.5.18 stop never swallowed : 1.5.19 exact-duration takes : 1.5.20 no-renumber + 55 audited fixes
```
...
| Version | Date | Notes |
|---|---|---|
| **1.5.20** | 2026-07-23 | **No-renumber sessions + all 55 audited firmware fixes.** An 84-agent adversarial audit found 55 confirmed defects (12 critical); fixed in six compile-gated phases, re-verified (56 verdicts, 0 still-broken). **Structural change:** sessions are never renumbered — numbers allocate monotonically and **wrap at 99**, delete removes only its own files and leaves a legal hole. This removed the entire renumber machinery and its critical cluster (renumber-under-upload splicing two takes, wrong-take deletion, power-cut slot reuse). **Mark-synced / keep-newest-5 audited end to end:** keep the 5 newest per patient, free an older take only if `.synced` AND server-byte-verified, **never free an unsynced take**; `writeSyncMarker()` parks the session on a write failure instead of spinning re-uploads. Also: OTA rollback restored + err-get-1 TLS-arena fix + OTA gated out of a take; crash-resume claims the SD bus before the net task can mark the take synced and honors the `record_seconds` cap; on-device status labels created (error messages were invisible); dead-mic silence detection; every `millis()` deadline made wrap-safe; BLE bridge streams from part files; `esp_reset_reason()` at boot + on the heartbeat + a serial `DIAG` dump; ~26.7 KB of buffers moved to PSRAM (static internal RAM 31%→24%). |
| **1.5.32** | 2026-07-24 | **Regression fixes from the full re-audit** (both were second-order effects of earlier fixes in the same code). The idle reclaim sweep could be starved by permanently-unverifiable synced takes consuming the shared verify budget every pass — such takes are now parked (a strike/cooldown) so the active dir is always reached; the verify gate is unchanged. And a crash-interrupted take whose resume gave up was left with no owner stamp, so the ownership gate hid its segments from upload forever — the give-up branches now stamp `owner_dev` so the segments still upload. Plus **system-wide error email alerts**: the 5-min status worker now emails the operator (via Cloudflare Email) on any new error — a service down, an AI/audio/processing error, a stuck job — and an all-clear when it resolves, fed by a new secret-gated `device-api` v18 `/health/alerts` digest. CI 7/7. |
| 1.5.31 | 2026-07-24 | **Comprehensive-audit fixes (firmware + web).** Firmware: each recording is stamped with its owning device id, so a device claimed to a NEW account never auto-uploads the prior account's on-card audio — closing a cross-clinic privacy leak (residual legacy audio uploads only via an explicit `resync_all` by whoever holds the claimed device). keep-newest-5 now ranks by a monotonic take sequence, so it keeps the genuinely newest recordings even after the session number wraps at 99. Web app: the fleet-OTA publish card is gated to admins on /devices; a per-recording signed-URL refresh budget (one recording's failures no longer wedge another's player); D-VoCD shows its real score for 35–36 token samples instead of a false 0.0; and the new **patient progress chart** on the Patient Detail → Analytics tab plots any clinical metric's variation across sessions. CI 7/7 on hardware; web deployed. |
| 1.5.30 | 2026-07-24 | **The last verified recorder + web-app fixes.** Firmware: offline crash-resume now starts the network task before the blocking resume, so a take resumed with Wi-Fi down at boot (a mains outage that took the room AP with it) stays stoppable — heartbeat, remote stop, BLE and the OTA health-confirm all keep running (the 1.5.17 fix only covered the online case). Sessions list shows only recordings with audio on the card, not synced-and-reclaimed tombstones (fw 1.5.29). Web app: closed the SALT round-trip regressions (surface spelling no longer shifts to the wrong word, inserted words get real timings, repetition/revision type is preserved), the fillerword double-count on save, and the failed-processing feedback dead-ends. CI gate 7/7 on hardware; web build clean. |
| 1.5.28 | 2026-07-23 | **Three audit rounds of recorder fixes (55 + 13 + 17 confirmed findings) and the retention rebuild.** Sessions are **never renumbered** — monotonic per directory, wrapping at 99 by recycling the oldest audio-free tombstone (without which a standalone unit stopped recording forever after 99 lifetime takes). **Keep-newest-5 is device-wide** and runs on an **idle sweep** across every directory, not only after an upload — which is why the card grew to 161 MB while Home said "all synced" — and still never frees a take the server has not byte-confirmed. **Standalone is the default target**: a roster arriving from the server is not an assignment. Every net-task SD path (remote commands, BLE ops, advertising refresh) now runs inside the SD arbitration bracket. Plus: OTA rollback restored, err-get-1 fixed, crash-resume corrected, on-device status messages made visible (they rendered to labels that were never created), dead-mic detection, all `millis()` deadlines wrap-safe, `esp_reset_reason()` on boot and heartbeat, a serial `DIAG` dump, and ~26.7 KB of buffers moved to PSRAM (largest contiguous internal block while recording 51 KB → 78 KB). |
