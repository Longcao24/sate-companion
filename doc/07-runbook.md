...
| `sate flash [recorder\|pendant]` | Build + flash; **target defaults to `recorder`** (bare `sate flash` works). Port auto-detect prefers the ESP32-S3 (Espressif VID `0x303A`) over other USB-serial devices and announces the chosen port (`--port` to override). `--debug` (recorder CDC serial), `--compile-only`, `--upload-only`, `--version 1.5.12` (flash a published older build to repro a field bug), `--image <bin>`. Pendant → `flash_xiao.sh` (Seeed core). |
