...
static const char    *FIRMWARE_VERSION  = "1.2.23-1c";  // single-core fallback build
...
// 1..N numbering contiguous (findNextSessionIndex assumes no gaps).
...
void loop()
{
  ...
  if (currentState != ERROR_STATE) {
    // *** 1_core build: FULLY SINGLE-CORE ***
    // The net task is NEVER started - connLoop() always runs here on the main loop,
    // exactly like the old SATE_Up. No core-0/core-1 concurrency, so nothing can
    // "stick" between the two cores. Trade-off: while a poll/upload HTTP call is in
    // flight the loop briefly blocks, but the ISR-latched buttons still catch the
    // press instantly and act on the next pass. This is the stable demo fallback.
    connLoop();
    renderUploadOverlay();
